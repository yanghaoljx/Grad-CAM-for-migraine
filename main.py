...
parser = argparse.ArgumentParser(description='PyTorch migraine Training')
...
trainset = MriDataset(mode='train',transform=transform_test)
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt4.pth')
        best_acc = acc
